...
from axon_conabio.models import TFModel
from axon_conabio.utils import get_checkpoints
from axon_conabio.trainer.base import Trainer


class TFTrainer(Trainer):
    base_model_class = TFModel

    def get_model_context(self):
        graph = tf.Graph()
        return {'graph': graph}

    def get_model_instance(self, context, model, model_kwargs):
        graph = context['graph']
    ...
    def build_training_pipeline(self, context, dataset_instance):
        graph = context['graph']
        ...
        return None, {'input': train_input, 'label': train_label}

    def build_validation_pipeline(self, context, dataset_instance):
        graph = context['graph']
        batch_size = int(self.config['feed']['batch_size'])
        epochs = int(self.config['feed']['epochs'])
        ...
        with graph.as_default():
            validation_input, validation_label = (
                dataset_instance.iter_validation(
                    batch_size=batch_size,
                    epochs=epochs))
        return None, {'input': validation_input, 'label': validation_label}

    def get_train_operation(self, model, losses, regularization_loss):
        ...
        with model.graph.as_default():
            ...
            if regularization_loss != 0:
                gradients_list += optimizer.compute_gradients(regularization_loss)

            train_op = optimizer.apply_gradients(
                gradients_list,
                global_step=model.global_step)

            total_loss = tf.add_n(losses) / num_gpus + regularization_loss

        return train_op, total_loss, gradients_list

    def build_losses(self, loss, model_instance, tensors):
        num_gpus = int(self.config['architecture']['num_gpus'])
        train_losses = loss.build_model_loss(
            model_instance,
            tensors['input'],
            tensors['label'],
            num_gpus=num_gpus,
            run_name='train')
        reg_loss = self.get_regularization_loss(model_instance)
        return train_losses, reg_loss
    ...
    def build_summary_op(
        ...
        if (grad_summs and gradients is not None):
            grad_summ_op = self.aggregate_gradients_summaries(gradients)
    ...
    def aggregate_gradients_summaries(self, gradients):
    ...
    def save_tensors(self, tensors, step):
    ...
    def train(
            ...
            loss_kwargs=None,
            dataset_kwargs=None):
        self.check_train_inputs(model, loss, dataset)
        ...
        context = self.get_train_context()

        model_instance = self.get_model_instance(context, model, model_kwargs)
        train_loss, validation_loss = self.get_loss_instances(
            context, loss, loss_kwargs)

        self.logger.info(
            'Building inputs',
            extra={'phase': 'construction'})
        dataset_instance = self.get_dataset_instance(context, dataset, dataset_kwargs)
        train_iterator, train_input_tensors = self.build_traning_pipeline(
            context, dataset_instance)

        validate = bool(self.config['summaries']['validate'])
        if validate:
            validate_iterator, validate_input_tensors = self.build_validation_pipeline(
                context, dataset_instance)
        ...
        self.logger.info(
            ...
            extra={'phase': 'construction'})
        train_losses, regularization_losses = self.build_losses(
            model_instance,
            train_loss,
            train_input_tensors)
        ...
        if validate:
            train_losses, _ = self.build_losses(
                model_instance,
                train_loss,
                train_input_tensors)
